...
object ScenarioFormTicket {
  ...
  val feeder = csv("login.csv").random //Рандом логин-пароль из файла (Необходимо перед тестом будет из создать)
  val numFeeder = Iterator.continually(Map("num" -> Random.between(1, 4))) //Рандом число от 1 до 3 (для выбора кол-ва пассажиров)
  //Даты вылета-Текущая+1день, прилета-Текущая+30 дней
  ...
  def scn5 = scenario("ScenarioFormTicket")
    .forever(
      pace(110)
        .exitBlockOnFail {
          exec(
            http("UC05_FormTicket_HomePage_0")
              .get("/WebTours/")
              .headers(headers_0)
              .resources(
                // Ломалась авторизация, пока закоментила
                //         http("request_1")
                //           .get("/WebTours/welcome.pl?signOff=true")
                //            .headers(headers_1),
                http("UC05_FormTicket_HomePage_2")
        ...
        .exitBlockOnFail {
          exec(
            http("UC05_FormTicket_Login_0")
              ...
              .resources(
                http("UC05_FormTicket_Login_1")
                  .get("/WebTours/nav.pl?page=menu&in=home")
                  .headers(headers_4)
                  .check(status.is(200)),
                http("UC05_FormTicket_Login_2")
        ...
        .exitBlockOnFail {
          exec(
            http("UC05_FormTicket_ClickFlights_0")
              .get("/WebTours/welcome.pl?page=search")
              .headers(headers_4)
              .check(status.is(200))
              .resources(
                http("UC05_FormTicket_ClickFlights_1")
                ...
                ,
                http("UC05_FormTicket_ClickFlights_2")
        ...
        .exitBlockOnFail {
          exec(
            http("UC05_FormTicket_FillDataTicket_0")
        ...
        .exitBlockOnFail {
          exec(
            http("UC05_FormTicket_ChooseTicket_0")
              .post("/WebTours/reservations.pl")
              .headers(headers_3)
        ...
        .doIfEqualsOrElse("#{num}", 1) {
          exitBlockOnFail {
            exec(
              http("UC05_FormTicket_Payment_0")
                .post("/WebTours/reservations.pl")
                .headers(headers_3)
        ...
        .exitBlockOnFail {
          exec(
            http("UC05_FormTicket_Logout_0")
              .get("/WebTours/welcome.pl?signOff=1")
              .headers(headers_4)
              .check(status.is(200))
              .resources(
                http("UC05_FormTicket_Logout_1")
                  .get("/WebTours/nav.pl?in=home")
                  .headers(headers_4)
                  .check(status.is(200))
              )
